...
- `live` 右ペインのミニ質問は `qa.answer.chunk` 単位で回答ブロックを段階追加し, `done` で確定
...
/                       ランディング兼デモ開始導線
/lectures               講義一覧
/lectures/:id/live      講義中ビュー(字幕, 翻訳, 資料, 板書, ミニ質問QA)
...
- 講義中は `/lectures/:id/live` 右ペインのミニ質問で根拠付きQAを実行
- 終了済みセッションは導線を持たず, 履歴として閲覧のみ
...
- 右上にゴミ箱アイコンを表示
  - クリック時は確認ダイアログを表示
  - `live` の場合は自動でセッション終了(`finalize`)後に削除
  - `ended` の場合は即時削除
  - 削除成功後は一覧と localStorage から除去
...
5. ミニ質問入力(任意)
   - 講義中は短文のみ
   - 回答は右ペイン内で根拠付きQAとして返す
   - 2問目以降は followup を使って文脈質問を処理する
...
## 8.4 ライブ右ペインのミニ質問(根拠付きQA)

### 目的

講義中に短文で質問し, ASR書き起こしを根拠として即時回答する.
review専用画面は持たず, live画面の右ペインで完結させる.
...
- QA回答ブロック
  - 回答本文
  - 根拠チップ
  - 音声根拠チップは対応する字幕ID (`S-xxx`) を先頭表示
  - エラー時の `再試行` / `再生成`

### 処理ルール

- 1問目は `POST /api/v4/lecture/qa/ask`
- 2問目以降は `POST /api/v4/lecture/qa/followup`
- QA索引は30秒ごとに `rebuild=true` で再構築して最新ASRを参照する
- 索引更新が失敗しても, 直前までの有効索引がある場合は警告表示して継続する
- 確定字幕には `S-001` 形式の連番IDを採番し, `citationId` の音声chunk参照から該当IDを解決してラベル化する
- QA回答本文中の音声参照(`[timestamp]`)も `S-xxx` に置換して表示する
...
- キーボードだけで `講義一覧 -> live -> ミニ質問送信` ができる
- focus ringが常に見える
