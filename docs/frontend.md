...
- 認証連携: Microsoft Entra ID(大学向け導線を意識)
...
/operator/session       デモ運営向けの入力確認(任意, 権限限定)
```
...
type LectureCard = {
  ...
  languageTags: string[] // ["JA", "EN support"]
  accessibilityTags: string[] // ["字幕", "翻訳", "文字拡大"]
  lastSummary?: string
}
```
...
- 初回ロード: skeleton card 6枚
- 0件: empty state
  - 文言: 何も登録されていない, ではなく, どの条件で0件かを書く
  - CTA: フィルタ解除
...
- 初版は右サイドシート
- URL同期不要
- LocalStorage保存 + ログイン時はサーバー同期(あれば)
...
GET    /api/lectures
GET    /api/lectures/:id
GET    /api/lectures/:id/review
GET    /api/lectures/:id/sources
POST   /api/lectures/:id/qa
GET    /api/lectures/:id/qa/:answerId
